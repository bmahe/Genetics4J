package net.bmahe.genetics4j.core.combination.gp;
...
public class ProgramRandomCombineHandler implements ChromosomeCombinatorHandler {
	...
	public ChromosomeCombinator resolve(final ChromosomeCombinatorResolver chromosomeCombinatorResolver,
		...
		return new ChromosomeCombinator() {
			...
			@SuppressWarnings("rawtypes")
			private final TreeNode<Operation<?>> mix(final TreeNode<Operation<?>> rootA,
					final TreeNode<Operation<?>> rootB, final Set<Class> acceptableClasses,
					final Map<Class, List<TreeNode<Operation<?>>>> returnedTypeToNodeA,
			...
			public List<Chromosome> combine(final Chromosome chromosome1, final Chromosome chromosome2) {
				...
				if (chromosome1 instanceof TreeChromosome<?> == false) {
					throw new IllegalArgumentException(
							"This mutator does not support chromosome of type " + chromosome1.getClass()
									.getSimpleName());
				}

				if (chromosome2 instanceof TreeChromosome<?> == false) {
					throw new IllegalArgumentException(
							"This mutator does not support chromosome of type " + chromosome2.getClass()
									.getSimpleName());
				}
